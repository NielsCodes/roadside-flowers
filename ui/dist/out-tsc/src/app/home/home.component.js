...
let HomeComponent = class HomeComponent {
    constructor(scripts, api, router, cookieService) {
        ...
        this.onResize();
        this.scripts.loadMusicKit().pipe(filter((status) => status === true)).subscribe((status) => {
    ...
    submitForm() {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            this.stage = 'save';
            yield this.api.registerData(d.from, d.to, d.message, this.dataId);
        });
    }
    onSpotifyLogin() {
    ...
    onAppleLogin() {
        ...
        if (hasSaved === 'true' && campaign === 'RF') {
            ...
            if (this.scripts.mkHasLoaded.getValue() === false) {
                this.scripts.mkHasLoaded.pipe(filter((status) => status === true)).subscribe((status) => {
                    this.loginWithApple();
                });
            }
            else {
