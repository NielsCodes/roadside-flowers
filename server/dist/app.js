...
const createImages = async (fromName, toName, message, id) => {
    ...
    const ctx = canvas.getContext('2d');
    const picture = await canvas_1.loadImage('./assets/picture-horizontal.png');
    ctx.drawImage(picture, 0, 0);
    ctx.font = '48px Ernie';
    ctx.textBaseline = 'top';
    // DRAW 'TO' NAME
 ...
 * @param id Data ID
 */
const createOutputImages = async (horizontalImage, id) => {
    /**
    ...
    const createHorizontalOutputImage = async () => {
        const canvas = canvas_1.createCanvas(1920, 1080);
        const ctx = canvas.getContext('2d');
        const background = await canvas_1.loadImage('./assets/output-horizontal.jpg');
    ...
    const createVerticalOutputImage = async () => {
        ...
        const ctx = canvas.getContext('2d');
        const background = await canvas_1.loadImage('./assets/output-vertical.jpg');
        ctx.drawImage(background, 0, 0);
        ctx.save();
        ctx.rotate(8 * Math.PI / 180);
